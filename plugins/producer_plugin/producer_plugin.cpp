   ...
   if( !_unapplied_transactions.empty() ) {
      ...
      while( itr != end_itr ) {
         ...
         try {
            ...
            if(!(_subjective_billing.is_disabled() || _subjective_billing.is_account_disabled(first_auth) || trx->read_only) && prev_billed_cpu_time_us > 0 && !rl.is_unlimited_cpu( first_auth )) {
